      ...
      {loading ? (
        ...
        <div className="space-y-6 animate-fade-in">
          {activeTab === 'daily' && data && (
            <>
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                <div className="card text-center animate-fade-in-up"><p className="text-sm text-gray-500">{UZ.totalSalesCount}</p><p className="text-2xl font-bold text-gray-900 dark:text-white">{data.summary?.total_sales || 0}</p></div>
                <div className="card text-center animate-fade-in-up stagger-1"><p className="text-sm text-gray-500">{UZ.totalRevenue}</p><p className="text-2xl font-bold text-emerald-600">{formatCurrency(data.summary?.total_revenue)}</p></div>
                <div className="card text-center animate-fade-in-up stagger-2"><p className="text-sm text-gray-500">{UZ.paymentMethods}</p><div className="flex justify-center gap-2 mt-1">{data.payment_breakdown?.map((p) => (<span key={p.payment_method} className="badge-info capitalize">{p.payment_method === 'cash' ? UZ.cash : p.payment_method === 'card' ? UZ.card : UZ.other}: {p.count}</span>))}</div></div>
              </div>
              <div className="card">
          ...
          {activeTab === 'monthly' && data && (
            <>
              <div className="card animate-fade-in-up"><p className="text-sm text-gray-500 mb-1">{UZ.monthlySummary}</p><p className="text-2xl font-bold text-gray-900 dark:text-white">{formatCurrency(data.summary?.total_revenue)}</p><p className="text-sm text-gray-500">{data.summary?.total_sales || 0} tranzaksiya</p></div>
              ...
              <div className="overflow-x-auto">
                <table className="w-full text-sm">
                  ...
                  <tbody className="divide-y divide-gray-50">{data.top_products?.map((p, i) => (
                    <tr key={p.id} className="hover:bg-gray-50 transition-colors"><td className="py-2 font-medium text-gray-500">{i + 1}</td><td className="py-2 font-medium text-gray-900 dark:text-white">{p.name}</td><td className="py-2 text-gray-500">{p.brand || '-'}</td><td className="py-2 text-right font-medium">{p.total_sold}</td><td className="py-2 text-right font-semibold">{formatCurrency(p.total_revenue)}</td></tr>
                  ))}</tbody>
                </table>
              </div>
            </div>
          ...
          {activeTab === 'inventory' && data && (
            <>
              <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
                <div className="card text-center animate-fade-in-up"><p className="text-sm text-gray-500">{UZ.totalProductsReport}</p><p className="text-2xl font-bold text-gray-900 dark:text-white">{data.summary?.total_products || 0}</p></div>
                <div className="card text-center animate-fade-in-up stagger-1"><p className="text-sm text-gray-500">{UZ.stockValue}</p><p className="text-2xl font-bold text-emerald-600">{formatCurrency(data.summary?.total_stock_value)}</p></div>
                <div className="card text-center animate-fade-in-up stagger-2"><p className="text-sm text-gray-500">{UZ.lowStockCount}</p><p className="text-2xl font-bold text-amber-600">{data.summary?.low_stock_count || 0}</p></div>
                <div className="card text-center animate-fade-in-up stagger-3"><p className="text-sm text-gray-500">{UZ.outOfStockCount}</p><p className="text-2xl font-bold text-red-600">{data.summary?.out_of_stock_count || 0}</p></div>
              </div>
              <div className="card">
                ...
                <div className="overflow-x-auto">
                  <table className="w-full text-sm">
                    ...
                    <tbody className="divide-y divide-gray-50">{data.products?.map((p) => (
                      <tr key={p.id} className="hover:bg-gray-50 transition-colors"><td className="py-2 font-medium text-gray-900 dark:text-white">{p.name}</td><td className="py-2 text-gray-500">{p.category_name || '-'}</td><td className="py-2 text-right font-medium">{p.stock_quantity} {p.unit}</td><td className="py-2 text-right text-gray-500">{p.minimum_stock}</td><td className="py-2 text-right">{formatCurrency(p.stock_value)}</td><td className="py-2">{p.stock_quantity === 0 ? <span className="badge-danger">{UZ.outOfStockStatus}</span> : p.stock_quantity <= p.minimum_stock ? <span className="badge-warning">{UZ.lowStockStatus}</span> : <span className="badge-success">{UZ.inStockStatus}</span>}</td></tr>
                    ))}</tbody>
                  </table>
                </div>
              </div>
