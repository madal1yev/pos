...
import { HiOutlineMinus, HiOutlinePlus, HiOutlineCamera, HiOutlineMagnifyingGlass, HiOutlineCheckCircle, HiOutlineXMark, HiOutlineShoppingCart, HiOutlineCalculator, HiOutlinePause, HiOutlinePlay } from 'react-icons/hi2';
import { Banknote, CreditCard, FileText, Smartphone, Truck, X, User, Receipt, CheckCircle } from 'lucide-react';
...
function CheckoutModal({ total, subtotal, taxAmount, taxRate, finalTotal, onClose, onComplete }) {
  ...
  const paymentMethods = [
    { value: 'cash', label: t('cash'), icon: Banknote, color: 'emerald' },
    { value: 'card', label: t('card'), icon: CreditCard, color: 'blue' },
    { value: 'debt', label: t('debt') || 'Qarz', icon: FileText, color: 'amber' },
    { value: 'other', label: t('other'), icon: Smartphone, color: 'purple' },
  ];

  return (
    <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center">
      <div className="fixed inset-0 bg-black/50 backdrop-blur-sm" onClick={onClose} />
      <div className="relative bg-white dark:bg-gray-800 rounded-t-2xl sm:rounded-2xl shadow-2xl w-full sm:max-w-md max-h-[95vh] sm:max-h-[90vh] flex flex-col">
        {/* Header */}
        <div className="flex items-center justify-between px-5 py-4 border-b border-gray-100 dark:border-gray-700 flex-shrink-0">
          ...
          <button onClick={onClose} className="p-2 rounded-lg hover:bg-gray-100 dark:hover:bg-gray-700 transition-colors"><X className="w-5 h-5" /></button>
        </div>
        ...
        <div className="flex-1 overflow-y-auto px-5 py-4 space-y-4 overscroll-contain">
          ...
          <div>
            ...
            <input type="text" value={customerName} onChange={(e) => setCustomerName(e.target.value)} className="w-full px-4 py-2.5 rounded-xl border border-gray-200 dark:border-gray-600 bg-white dark:bg-gray-700 text-gray-900 dark:text-white focus:border-indigo-500 focus:ring-2 focus:ring-indigo-500/20 outline-none transition-all" placeholder={t('walkInCustomer')} />
          </div>
          ...
          {showDelivery && (
            <div className="animate-fade-in">
              <label className="flex items-center gap-2 text-sm font-medium text-gray-700 dark:text-gray-300 mb-1.5">
                <Truck className="w-4 h-4" /> Yetkazib berish manzili
              </label>
              <textarea value={deliveryAddress} onChange={(e) => setDeliveryAddress(e.target.value)} className="w-full px-4 py-2.5 rounded-xl border border-gray-200 dark:border-gray-600 bg-white dark:bg-gray-700 text-gray-900 dark:text-white focus:border-indigo-500 focus:ring-2 focus:ring-indigo-500/20 outline-none transition-all resize-none" placeholder="Manzilni kiriting..." rows={2} />
            </div>
          )}
            ...
            <div className="grid grid-cols-2 gap-2">
              {paymentMethods.map(({ value, label, icon: Icon, color }) => (
                <button key={value} onClick={() => setPaymentMethod(value)}
                  className={`flex items-center gap-3 p-3.5 rounded-xl border-2 transition-all ${paymentMethod === value
                    ? `border-${color}-500 bg-${color}-50 dark:bg-${color}-900/20 shadow-md shadow-${color}-500/10`
                    : 'border-gray-200 dark:border-gray-700 hover:border-gray-300 dark:hover:border-gray-600'}`}>
                  <div className={`w-9 h-9 rounded-lg flex items-center justify-center ${paymentMethod === value ? `bg-${color}-100 dark:bg-${color}-900/30` : 'bg-gray-100 dark:bg-gray-700'}`}>
                    <Icon className={`w-5 h-5 ${paymentMethod === value ? `text-${color}-600 dark:text-${color}-400` : 'text-gray-500 dark:text-gray-400'}`} />
                  </div>
                  <span className={`text-sm font-medium ${paymentMethod === value ? `text-${color}-700 dark:text-${color}-400` : 'text-gray-600 dark:text-gray-400'}`}>{label}</span>
                </button>
              ))}
            </div>
          </div>
          ...
          {paymentMethod === 'cash' && (
            <div className="space-y-3 animate-fade-in">
              <div>
                ...
                <input type="number" value={receivedAmount} onChange={(e) => setReceivedAmount(e.target.value)} className="w-full px-4 py-3 rounded-xl border-2 border-gray-200 dark:border-gray-600 bg-white dark:bg-gray-700 text-xl font-bold text-gray-900 dark:text-white focus:border-indigo-500 focus:ring-2 focus:ring-indigo-500/20 outline-none transition-all" />
              </div>
              <div>
        ...
        <div className="flex-shrink-0 p-4 border-t border-gray-100 dark:border-gray-700">
          <button onClick={handleComplete} disabled={processing} className="w-full bg-gradient-to-r from-indigo-600 to-indigo-700 text-white py-3.5 rounded-xl text-base font-semibold hover:from-indigo-700 hover:to-indigo-800 transition-all disabled:opacity-50 shadow-lg shadow-indigo-500/25 active:scale-[0.98] flex items-center justify-center gap-2">
