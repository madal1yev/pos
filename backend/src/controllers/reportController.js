...
async function safeQuery(label, queryFn) {
  try {
    ...
    console.error(`Report query "${label}" failed:`, error.message);
    return { rows: [] };
  }
}
...
exports.daily = async (req, res, next) => {
  try {
    ...
    res.json({
      date: targetDate,
      summary: summary.rows[0] || { total_sales: 0, total_revenue: 0, total_change: 0 },
...
exports.monthly = async (req, res, next) => {
  try {
    ...
    const [dailySales, monthlySummary] = await Promise.all([
    ...
    res.json({
      year: yearNum,
      month: monthNum,
      summary: monthlySummary.rows[0] || { total_sales: 0, total_revenue: 0, net_revenue: 0 },
...
exports.inventory = async (req, res, next) => {
  try {
    ...
    res.json({ summary: summary.rows[0] || { total_products: 0, total_stock: 0, total_stock_value: 0, low_stock_count: 0, out_of_stock_count: 0 }, products: result.rows || [] });
  } catch (error) {
    next(error);
  }
};
