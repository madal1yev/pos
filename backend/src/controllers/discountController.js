...
exports.getAll = async (req, res, next) => {
  try {
    ...
    let where = 'd.store_id = $1';
    let params = [req.user.store_id];
    if (is_active !== undefined) {
      where += ' AND d.is_active = $2';
...
exports.getById = async (req, res, next) => {
  try {
    const result = await db.query('SELECT * FROM discounts WHERE id = $1 AND store_id = $2', [req.params.id, req.user.store_id]);
...
exports.create = async (req, res, next) => {
  try {
    ...
    const result = await db.query(
      `INSERT INTO discounts (name, type, value, min_purchase, max_discount, start_date, end_date, store_id)
       VALUES ($1, $2, $3, $4, $5, $6, $7, $8) RETURNING *`,
      [name, type, parseFloat(value), parseFloat(min_purchase) || 0, max_discount ? parseFloat(max_discount) : null, start_date || null, end_date || null, req.user.store_id]
    );

    // Log audit - safely handle if table doesn't exist
    try {
      await db.query(
        `INSERT INTO audit_logs (user_id, username, action, entity_type, entity_id, new_value, store_id)
         VALUES ($1, $2, 'discount_create', 'discounts', $3, $4, $5)`,
        [req.user.id, req.user.name, result.rows[0].id, JSON.stringify({ name, type, value }), req.user.store_id]
      );
    } catch (auditErr) {
...
exports.update = async (req, res, next) => {
  try {
    const current = await db.query('SELECT * FROM discounts WHERE id = $1 AND store_id = $2', [req.params.id, req.user.store_id]);
    ...
    try {
      await db.query(
        `INSERT INTO audit_logs (user_id, username, action, entity_type, entity_id, old_value, new_value, store_id)
         VALUES ($1, $2, 'discount_update', 'discounts', $3, $4, $5, $6)`,
        [req.user.id, req.user.name, req.params.id,
          JSON.stringify(current.rows[0]), JSON.stringify(result.rows[0]), req.user.store_id]
      );
    } catch (auditErr) {
      // audit_logs table might not exist yet, ignore
    }
...
exports.remove = async (req, res, next) => {
  try {
    const existing = await db.query('SELECT id FROM discounts WHERE id = $1 AND store_id = $2', [req.params.id, req.user.store_id]);
    if (existing.rows.length === 0) return res.status(404).json({ error: 'Chegirma topilmadi' });
...
exports.createPromoCode = async (req, res, next) => {
  try {
    ...
    const owned = await db.query('SELECT id FROM discounts WHERE id = $1 AND store_id = $2', [discount_id, req.user.store_id]);
    if (owned.rows.length === 0) return res.status(404).json({ error: 'Chegirma topilmadi' });
    const result = await db.query(
      `INSERT INTO promo_codes (code, discount_id, max_uses, store_id)
       VALUES ($1, $2, $3, $4) RETURNING *`,
      [code.toUpperCase(), discount_id, max_uses || 0, req.user.store_id]
    );
...
exports.validatePromoCode = async (req, res, next) => {
  try {
    ...
    const result = await db.query(
      ...
      [code.toUpperCase(), req.user.store_id]
    );
