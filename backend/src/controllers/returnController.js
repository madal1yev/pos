...
exports.returnProducts = async (req, res) => {
  try {
    ...
    for (const item of items) {
      ...
      const product = await db.query('SELECT stock_quantity FROM products WHERE id = $1', [productId]);
      if (product.rows.length > 0) {
        ...
        await db.query(
          ...
          [productId, returnQty, currentStock, newStock, `Qaytarish: ${saleData.invoice_number}`, req.user?.id || 1]
        );
      }
    }
    ...
    if (totalReturnAmount > 0) {
      await db.query(
        "INSERT INTO refunds (sale_id, user_id, refund_amount, reason, status) VALUES ($1, $2, $3, $4, 'completed')",
...
exports.getSaleForReturn = async (req, res) => {
  try {
    ...
    if (!saleId) {
      return res.status(400).json({ error: 'Noto\'g\'ri savdo ID' });
    }
